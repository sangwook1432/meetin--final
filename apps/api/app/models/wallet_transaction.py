...
설계 원칙:
  - 모든 잔액 변동(충전/차감/환불/몰수)은 반드시 이 테이블에 기록.
  - user.balance 는 항상 이 테이블의 sum 과 일치해야 함.
  - 충전: tx_type=CHARGE, amount=양수
  - 보증금 차감: tx_type=DEPOSIT_HOLD, amount=음수
  - 보증금 환불: tx_type=DEPOSIT_REFUND, amount=양수
  - 잔액 반환(출금): tx_type=WITHDRAW, amount=음수
"""
